// ================================================================
// KERN RIVER RUN - game-pixel-experiment.js
// Isolated pixelation post-processing experiment.
// Renders the scene to a low-res WebGLRenderTarget, then upscales
// to screen with NearestFilter for a pixel-art look.
// SOURCE: copied from game-3d.js -- do NOT edit game-3d.js here.
// ================================================================
...
// ── PIXEL EXPERIMENT: change PIXEL_SIZE to tune chunkiness ────────
// 4 = render at 1/4 res, upscale 4x. Raise for more pixelation.
const PIXEL_SIZE = 4;
const CAM_Y     = 4.8;
const CAM_Z_BK  = 8.5;
...
// ── PIXEL PASS SETUP ─────────────────────────────────────────────
// pixelRT: off-screen render target at reduced resolution.
// pixelScene/pixelCam: fullscreen quad that upscales pixelRT to screen
// using NearestFilter, producing the chunky pixel-art look.
let pixelRT    = null;
let pixelScene = null;
const pixelCam = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1);

function buildPixelPass(w, h) {
  var dpr = renderer.getPixelRatio();
  var rtW = Math.max(1, Math.floor(w * dpr / PIXEL_SIZE));
  var rtH = Math.max(1, Math.floor(h * dpr / PIXEL_SIZE));
  if (pixelRT) pixelRT.dispose();
  pixelRT = new THREE.WebGLRenderTarget(rtW, rtH, {
  ...
  if (!pixelScene) {
    ...
    var qm = pixelScene.children[0].material;
    qm.map = pixelRT.texture;
    qm.needsUpdate = true;
  }
}
...
function loop3() {
  ...
  updateVisuals3();
  // Pass 1: render full 3D scene to low-res off-screen target
  renderer.setRenderTarget(pixelRT);
  renderer.render(scene, camera);
  // Pass 2: upscale to screen via fullscreen quad (NearestFilter = pixel-art look)
  renderer.setRenderTarget(null);
  renderer.render(pixelScene, pixelCam);
}
