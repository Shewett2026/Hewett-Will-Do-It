...
const SCROLL_SPEED        = 3;      // px per frame — river scroll & obstacle movement
...
// Higher = more frequent obstacles; keep below 0.04 to avoid impossible runs.
const OBSTACLE_FREQUENCY  = 0.022;  // per-frame spawn probability
...
const MIN_OBSTACLE_GAP    = 135;    // px; at SCROLL_SPEED=3 ≈ 45 frames between spawns
...
const player = {
  ...
  animFrame:   0,    // runs continuously for leg/bob animation
  dead:        false,
};
...
function initBgDecor() {
  ...
  for (let i = 0; i < 26; i++) {
    bgDecor.push({
      ...
      offX:  bw * (0.1 + Math.random() * 0.8), // offset within the bank strip
      y:     Math.random() * poolH,
      sz:    5 + Math.random() * 9,
...
function drawBackground() {
  ...
  // Water gradient — deeper blue at edges, lighter in center (light refraction)
  ...
  // Scrolling bank trees and rocks
  drawBankDecor();
}
...
function drawLog(obs) {
  ...
  // Label so player can read incoming full-width obstacle
...
function drawLaneObs(obs) {
  ...
  if (obs.type === 'rock') {
    ...
    // White-capped rapids — wider than a point obstacle, fills lane
...
// ── DRAW: PLAYER ────────────────────────────────────────��─────────
function drawPlayer() {
  const PW  = 15;   // base pixel width
  const PH  = 20;   // base pixel height
  const s   = player.jumpScale;
  const px  = player.x;
  const py  = player.y;
  const leg = (player.animFrame % 22) < 11; // leg alternation phase

  // Shadow — shrinks toward zero at jump apex
  ctx.save();
  ctx.globalAlpha = player.isJumping ? 0.2 * player.shadowScale : 0.28;
  ctx.fillStyle   = '#000';
  ctx.beginPath();
  ctx.ellipse(px, py + PH * 0.52, PW * 0.62 * (player.isJumping ? player.shadowScale : 1),
    PW * 0.2, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();

  // All player drawing is scaled from its center
  ctx.save();
  ctx.translate(px, py);
  ctx.scale(s, s);

  // Body — campaign navy jacket from overhead
  ctx.fillStyle = '#0B1F3A';
  ctx.fillRect(-PW / 2, -PH / 2, PW, PH * 0.62);

  // Head — visible above shoulders
  ctx.fillStyle = '#FBBF24';
  ctx.fillRect(-PW * 0.3, -PH / 2 - PW * 0.38, PW * 0.6, PW * 0.5);

  // Backpack detail
  ctx.fillStyle = '#1E3A5F';
  ctx.fillRect(-PW * 0.2, -PH * 0.12, PW * 0.4, PH * 0.26);

  // Running legs — alternate each ~11 frames for a stride effect
  ctx.fillStyle = '#374151';
  if (leg) {
    ctx.fillRect(-PW * 0.42, PH * 0.08,  PW * 0.3, PH * 0.36);
    ctx.fillRect( PW * 0.12, PH * 0.18,  PW * 0.3, PH * 0.26);
  } else {
    ctx.fillRect(-PW * 0.42, PH * 0.18,  PW * 0.3, PH * 0.26);
    ctx.fillRect( PW * 0.12, PH * 0.08,  PW * 0.3, PH * 0.36);
  }

  // Arms — swing opposite to legs
  ctx.fillStyle = '#0B1F3A';
  const as = leg ? 1 : -1;
  ctx.fillRect(-PW * 0.57,  -PH * 0.08 + as * 3, PW * 0.14, PH * 0.28);
  ctx.fillRect( PW * 0.43,  -PH * 0.08 - as * 3, PW * 0.14, PH * 0.28);
...
function drawHUD() {
  ...
  // Row 1: stage name · mile counter · stage badge
  ...
  // Row 2: running score · high score
...
function update() {
  ...
  // Smooth player x toward current target lane
...
function checkCollision() {
  const py   = player.y;
  const half = 12; // vertical half-size of player collision box

  for (const obs of obstacles) {
    if (obs.resolved) continue;

    // Check if obstacle overlaps the player's vertical collision zone
    if (obs.y + obs.h >= py - half && obs.y <= py + half) {
      obs.resolved = true; // only evaluate once per obstacle
...
function drawStartScreen() {
  ...
  // Sky-to-river gradient background
  ...
  // ── "Back to Campaign" link — NOTE: swap text for a PNG logo here in a later phase
  ctx.font         = '6px "Press Start 2P", monospace';
  ...
  // Title — gold "KERN RIVER" + cream "RUN"
  ctx.fillStyle = '#C9883A';
...
function drawHowToPlay() {
  ...
  const lines = [
    ['Move left/right between',  '#F5F0E8'],
...
function drawGameOver() {
  ...
  // Semi-transparent overlay so the last game frame shows through
  ...
  if (stageComplete) {
    ...
    ctx.font      = '6px "Press Start 2P", monospace';
    ctx.fillStyle = '#F5F0E8';
    ctx.fillText('You ran the Kern River', W / 2, H * 0.22 + 48);
...
// ── INPUT: GAMEPLAY ───────────────────────────────────────────────
function doMoveLeft()  { if (player.targetLane > 0)              player.targetLane--; }
function doMoveRight() { if (player.targetLane < LANE_COUNT - 1) player.targetLane++; }
function doJump() {
